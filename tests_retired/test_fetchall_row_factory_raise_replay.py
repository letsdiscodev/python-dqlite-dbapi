"""``fetchall`` applies ``_row_factory`` before advancing ``_row_index``.

A factory that raises must leave the index unchanged so the rows can be re-fetched.
"""
...
def test_sync_fetchall_factory_raise_does_not_advance_index() -> None:
    """A factory raise leaves ``_row_index`` unchanged so a retry returns all rows."""
...
def test_sync_fetchall_factory_raise_midstream_index_unchanged() -> None:
    """Factory raises on the 2nd of 3 rows; fetchall aborts with no partial index advance."""
...
async def test_async_fetchall_factory_raise_does_not_advance_index() -> None:
    """Async-sibling parity: relies on the comprehension raising before the index advance."""
