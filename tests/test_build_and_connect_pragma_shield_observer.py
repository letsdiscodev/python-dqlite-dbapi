"""Pin: the PRAGMA query_only cleanup arm hoists conn.close() into an explicit Task
+ observer before shielding, so an outer cancel mid-await does not orphan a Task
(which would surface as "Task exception was never retrieved" at GC)."""
...
async def test_pragma_cleanup_shield_uses_observer_not_bare_coro() -> None:
    """If the PRAGMA emit raises, cleanup conn.close() is hoisted into a Task before shield."""
    ...
    # Fake conn whose execute raises (forcing the PRAGMA cleanup arm).
    fake_conn = MagicMock()
