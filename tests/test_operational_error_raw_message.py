"""Pin: dbapi exceptions plumb the un-truncated server text through ``raw_message``."""
...
async def test_operational_error_preserves_raw_message_through_call_client() -> None:
    ...
    # ``message`` is truncated at the client layer (1 KiB cap).
    assert len(ei.value.args[0]) < len(long_msg)
    assert "truncated" in ei.value.args[0]
    assert ei.value.code == 1
    # ``raw_message`` is capped at 4 KiB to bound hostile-peer payloads in
    # pickled exception graphs.
...
def test_interface_error_carries_code_and_raw_message() -> None:
    """InterfaceError carries ``code``/``raw_message`` so protocol faults keep them."""
...
def test_interface_error_default_raw_message_is_message() -> None:
    """raw_message defaults to message so callers always see a non-empty field."""
