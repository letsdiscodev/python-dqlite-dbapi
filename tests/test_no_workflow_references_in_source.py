...
# matched only with a capital ``P`` so the legitimate lowercase CPython
# ``Py_FinalizeEx`` "phase 3" interpreter-shutdown references are NOT
# flagged.
...
def _shipped_source_files() -> list[pathlib.Path]:
    package_dir = pathlib.Path(dqlitedbapi.__file__).parent
