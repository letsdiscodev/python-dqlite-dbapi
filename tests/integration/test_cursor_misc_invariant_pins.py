"""Cursor invariant pins: description tuple identity, fetchall idempotency,
closed cursor's .connection resolving while the parent Connection is alive."""
...
def cur_with_rows() -> Iterator[dqlitedbapi.Cursor]:
    """A cursor primed with three rows so description comes from a real wire response."""
...
def test_description_returns_same_tuple_object_across_accesses(
    ...
    """``cur.description`` returns the same tuple object on each access (no lazy rebuild)."""
...
def test_closed_cursor_dot_connection_still_resolves_while_conn_alive() -> None:
    """A closed cursor's ``.connection`` still resolves while the parent Connection is alive."""
    conn = dqlitedbapi.connect("localhost:9001", database="test_misc_pins")
    try:
        cur = conn.cursor()
        cur.close()
